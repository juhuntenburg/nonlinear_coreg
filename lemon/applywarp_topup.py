...
# workflow
applywarp_topup = Workflow(name='applywarp_topup')
...
# outputnode                                     
outputnode=Node(util.IdentityInterface(fields=['topup_mean_fullwarped',
                                               #'lin_ts_fullwarped'
                                               ]),
                name='outputnode')
...
applywarp_topup.connect([(inputnode, transformlist, [('norm_lin', 'trans2'),
                                                      ('norm_warp', 'trans1')])
                          ])
...
applytransform = Node(ants.ApplyTransforms(dimension=3,
                                           output_image='topup_mean_norm.nii.gz',
                                           interpolation = 'BSpline'),
                      'applytransform')
 
applywarp_topup.connect([(inputnode, applytransform, [('epi_coreg', 'input_image')]),
                          (inputnode, applytransform, [('mni', 'reference_image')]),
                          (transformlist, applytransform, [('transformlist', 'transforms')]),
                          (applytransform, outputnode, [('output_image', 'topup_mean_fullwarped')])
                          ])
...
applywarp_topup.base_dir='/scr/kansas1/huntenburg/lemon_missing/working_dir/'
...
applywarp_topup.config['execution']['crashdump_dir'] = applywarp_topup.base_dir + "/crash_files"
...
# select files
templates={'epi_coreg':'preprocessed/{subject_id}/topup_coreg/topup_mean_coreg.nii.gz',
...
applywarp_topup.connect([(infosource, selectfiles, [('subject_id', 'subject_id')]),
                          ...
                          (selectfiles, inputnode, [('epi_coreg', 'epi_coreg'),
                                                    ...
                                                    ('norm_lin', 'norm_lin'),
                                                    ('norm_warp', 'norm_warp')]),
                          (outputnode, sink, [('topup_mean_fullwarped', 'topup_coreg.@topup_mean_norm'),
                                              #('lin_ts_fullwarped', 'coreg.@lin_ts_fullwarped')
                                              ])
                ])

applywarp_topup.run(plugin='CondorDAGMan')