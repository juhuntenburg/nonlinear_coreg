...
# workflow
applywarp_fmap = Workflow(name='applywarp_fmap')
...
# outputnode                                     
outputnode=Node(util.IdentityInterface(fields=['fmap_mean_fullwarped',
                                               #'lin_ts_fullwarped'
                                               ]),
                name='outputnode')
...
applywarp_fmap.connect([(inputnode, transformlist, [('norm_lin', 'trans2'),
                                                      ('norm_warp', 'trans1')])
                          ])
...
applytransform = Node(ants.ApplyTransforms(dimension=3,
                                           output_image='fmap_mean_norm.nii.gz',
                                           interpolation = 'BSpline'),
                      'applytransform')
 
applywarp_fmap.connect([(inputnode, applytransform, [('epi_coreg', 'input_image')]),
                          (inputnode, applytransform, [('mni', 'reference_image')]),
                          (transformlist, applytransform, [('transformlist', 'transforms')]),
                          (applytransform, outputnode, [('output_image', 'fmap_mean_fullwarped')])
                          ])
...
# select files
templates={'epi_coreg':'preprocessed/{subject_id}/fieldmap_coreg/fmap_mean_coreg.nii.gz',
...
applywarp_fmap.connect([(infosource, selectfiles, [('subject_id', 'subject_id')]),
                          ...
                          (selectfiles, inputnode, [('epi_coreg', 'epi_coreg'),
                                                    ...
                                                    ('norm_lin', 'norm_lin'),
                                                    ('norm_warp', 'norm_warp')]),
                          (outputnode, sink, [('fmap_mean_fullwarped', 'fieldmap_coreg.@fmap_mean_norm'),
                                              #('lin_ts_fullwarped', 'coreg.@lin_ts_fullwarped')
                                              ])
                ])

applywarp_fmap.run(plugin='CondorDAGMan')